...
def admin_dashboard(request):
    ...
    fuel_purchases_month_qs = SiteFuelPurchase.objects.filter(
        site__actif=True,
        billing_month=selected_water_month,
    ).select_related("site", "created_by").order_by("-purchase_date", "-created_at")
    recent_fuel_purchases = list(
        SiteFuelPurchase.objects.filter(site__actif=True)
        .select_related("site", "created_by")
        .order_by("-billing_month", "-purchase_date", "-created_at")[:6]
    )
    fuel_purchase_summary = {
        "selected_month": selected_water_month,
        "month_total": fuel_purchases_month_qs.aggregate(total=Sum("amount_fc"))["total"] or Decimal("0"),
        "month_count": fuel_purchases_month_qs.count(),
        "site_breakdown": list(
            fuel_purchases_month_qs.values("site__nom")
            .annotate(total=Sum("amount_fc"), count=Count("id"))
            .order_by("-total", "site__nom")
        ),
    }
...
def _build_admin_fuel_purchase_context(selected_month, form):
    today = timezone.localdate()
    purchase_qs = (
        SiteFuelPurchase.objects.filter(site__actif=True, billing_month=selected_month)
        .select_related("site", "created_by")
        .order_by("-purchase_date", "-created_at")
    )
    purchases = list(purchase_qs)
    ...
    selected_sites_count = len(by_site)
    previous_month = _previous_month_start(selected_month)
    previous_month_qs = SiteFuelPurchase.objects.filter(site__actif=True, billing_month=previous_month)
    previous_month_total = previous_month_qs.aggregate(total=Sum("amount_fc"))["total"] or Decimal("0")
    previous_month_count = previous_month_qs.count()
    month_total_delta = month_total - previous_month_total
    monthly_history = list(
        SiteFuelPurchase.objects.filter(site__actif=True)
        .values("billing_month")
    ...
    recent_purchases = list(
        SiteFuelPurchase.objects.filter(site__actif=True)
        .select_related("site", "created_by")
        .order_by("-billing_month", "-purchase_date", "-created_at")[:8]
    )
    top_site = by_site[0] if by_site else None
...
@login_required
@no_cache_view
def admin_fuel_purchases(request):
    user = request.user
    ensure_superuser_admin_profile(user)
    ...
    if request.method == "POST":
        form = SiteFuelPurchaseForm(request.POST)
        if form.is_valid():
            purchase = form.save(commit=False)
            purchase.created_by = user
            purchase.save()
            messages.success(request, "L'achat de carburant a été enregistré.")
            month_query = purchase.billing_month.strftime("%Y-%m")
            return redirect(f"{reverse('admin_fuel_purchases')}?month={month_query}")
    else:
        form = SiteFuelPurchaseForm(initial={"billing_month": selected_month})

    return render(
        request,
        "admin/fuel_purchases.html",
        _build_admin_fuel_purchase_context(
            selected_month=selected_month,
            form=form,
        ),
    )
...
@login_required
@no_cache_view
def admin_edit_fuel_purchase(request, purchase_id):
    user = request.user
    ensure_superuser_admin_profile(user)
    ...
    if request.method == "POST":
        form = SiteFuelPurchaseForm(request.POST, instance=purchase)
        if form.is_valid():
            updated_purchase = form.save()
            messages.success(request, "L'achat de carburant a été mis à jour.")
            month_query = updated_purchase.billing_month.strftime("%Y-%m")
            return redirect(f"{reverse('admin_fuel_purchases')}?month={month_query}")
    else:
        form = SiteFuelPurchaseForm(instance=purchase)

    return render(
        request,
        "admin/edit_fuel_purchase.html",
        {
            "purchase": purchase,
            "form": form,
        },
...
def admin_delete_fuel_purchase(request, purchase_id):
    ...
    if request.method == "POST":
        ...
        if redirect_url:
            return redirect(redirect_url)
        if request.GET.get("from_site") == "1":
