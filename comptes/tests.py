...
class LoginPageTests(TestCase):
    def test_login_page_uses_default_title_layout(self):
        ...
        self.assertNotContains(response, 'login-title--world-cup', html=False)
        self.assertNotContains(response, 'login-worldcup-ball', html=False)
...
class FuelPurchaseTrackingTests(TestCase):
    def setUp(self):
        self.admin_user = User.objects.create_superuser(
            username="fuel_admin",
            email="fuel_admin@example.com",
            password="AdminPass123!",
        )
        self.site = Location.objects.create(
            nom="Site Carburant",
            adresse="Adresse Carburant",
            ville="Kinshasa",
            actif=True,
        )
        self.client.login(username="fuel_admin", password="AdminPass123!")

    def test_admin_can_create_fuel_purchase(self):
        response = self.client.post(
            reverse("admin_fuel_purchases"),
            data={
                "site": str(self.site.id),
                "billing_month": "2026-07",
                "purchase_date": "2026-07-04",
                "amount_fc": "18500",
                "notes": "Carburant du générateur",
            },
        )
        ...
        self.assertRedirects(
            response,
            f"{reverse('admin_fuel_purchases')}?month=2026-07",
            fetch_redirect_response=False,
        )

        purchase = SiteFuelPurchase.objects.get(site=self.site)
        self.assertEqual(purchase.billing_month, date(2026, 7, 1))
        self.assertEqual(purchase.purchase_date, date(2026, 7, 4))
        self.assertEqual(purchase.amount_fc, Decimal("18500"))
        self.assertEqual(purchase.created_by, self.admin_user)

    def test_admin_can_edit_fuel_purchase(self):
        purchase = SiteFuelPurchase.objects.create(
            site=self.site,
            billing_month=date(2026, 7, 1),
            purchase_date=date(2026, 7, 4),
            amount_fc=Decimal("18500"),
            notes="Ancienne note",
            created_by=self.admin_user,
        )

        response = self.client.post(
            reverse("admin_edit_fuel_purchase", args=[purchase.id]),
            data={
                "site": str(self.site.id),
        ...
        self.assertRedirects(
            response,
            f"{reverse('admin_fuel_purchases')}?month=2026-08",
            fetch_redirect_response=False,
        )

        purchase.refresh_from_db()
        self.assertEqual(purchase.billing_month, date(2026, 8, 1))
        self.assertEqual(purchase.purchase_date, date(2026, 8, 2))
        self.assertEqual(purchase.amount_fc, Decimal("21500"))
        self.assertEqual(purchase.notes, "Montant corrigé par admin")

    def test_fuel_purchase_page_filters_selected_month_and_shows_actions(self):
        hidden_purchase = SiteFuelPurchase.objects.create(
            site=self.site,
            billing_month=date(2026, 6, 1),
            purchase_date=date(2026, 6, 28),
            amount_fc=Decimal("17000"),
            notes="Achat juin",
            created_by=self.admin_user,
        )
        visible_purchase = SiteFuelPurchase.objects.create(
            site=self.site,
            billing_month=date(2026, 7, 1),
            purchase_date=date(2026, 7, 4),
            amount_fc=Decimal("18500"),
        ...
        response = self.client.get(reverse("admin_fuel_purchases"), data={"month": "2026-07"})

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Gestion du carburant")
        self.assertContains(response, "Achat juillet")
        self.assertContains(response, reverse("admin_edit_fuel_purchase", args=[visible_purchase.id]))
        self.assertContains(response, reverse("admin_delete_fuel_purchase", args=[visible_purchase.id]))
        self.assertContains(response, f"{reverse('admin_fuel_purchases')}?month=2026-06")
        self.assertEqual(response.context["month_total"], Decimal("18500"))
        self.assertEqual(response.context["month_count"], 1)
        self.assertEqual(len(response.context["purchases"]), 1)
        self.assertEqual(response.context["purchases"][0].id, visible_purchase.id)
        self.assertEqual(hidden_purchase.billing_month, date(2026, 6, 1))

    def test_admin_dashboard_shows_fuel_purchase_summary(self):
        SiteFuelPurchase.objects.create(
            site=self.site,
            billing_month=date(2026, 7, 1),
            purchase_date=timezone.localdate(),
            amount_fc=Decimal("19500"),
            notes="Achat carburant du jour",
            created_by=self.admin_user,
        )
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Achats de carburant")
        self.assertContains(response, "19 500")
        self.assertContains(response, reverse("admin_fuel_purchases"))
